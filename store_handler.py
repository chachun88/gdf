	...
	def get(self):
		...
		if "success" in res:
			if ajax == 0:
				...
				self.write(json_util.dumps({"html":self.render_string("store/ajax_productos.html",data=res["success"],url_bodega=url_bodega,money_format=self.money_format,items=items,page=page),"items":items,"page":page}))
		else:
